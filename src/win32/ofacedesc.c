...
struct __GLCdataRec {
  __GLCmaster* master;
  __GLCfaceDesc* faceDesc;
  LPCSTR faceName;
};



static int CALLBACK __glcEnumFaceDescCreate(ENUMLOGFONTEX* inElfe,
                                            ...
                                            DWORD inFontType, LPARAM inData)
{
  ...
  if (lstrcmp(inElfe->elfStyle, data->faceName))
    return 1;

  data->faceDesc->font = CreateFontIndirect(&inElfe->elfLogFont);
  if (FAILED(data->faceDesc->font)) {
    __glcRaiseError(GLC_RESOURCE_ERROR);
    return 1;
  }

  if (FAILED(SelectFont(dc, data->faceDesc->font))) {
    DeleteObject(data->faceDesc->font);
    __glcRaiseError(GLC_RESOURCE_ERROR);
    return 1;
  }
  ...
  data->faceDesc->pattern = (LPGLYPHSET)__glcMalloc(size);
  if (!data->faceDesc->pattern) {
    DeleteObject(font);
    __glcRaiseError(GLC_RESOURCE_ERROR);
    return 1;
  }

  if (!GetFontUnicodeRanges(dc, data->faceDesc->pattern)) {
    DeleteObject(font);
    __glcFree(data->faceDesc->pattern);
    __glcRaiseError(GLC_RESOURCE_ERROR);
    return 1;
  }
					 ...
					 __GLCcontext* inContext)
{
...
#ifdef UNICODE
  size = MultiByteToWideChar(CP_UTF8, 0, inFace, -1, NULL, 0);

  data.faceName = (LPWSTR)__glcMalloc(size * sizeof(WCHAR));
  if (!data.faceName) {
    __glcRaiseError(GLC_RESOURCE_ERROR);
    DeleteDC(dc);
    __glcFree(This);
  ...
  MultiByteToWideChar(CP_UTF8, 0, inFace, -1, data.faceName, size);
#else
  data.faceName = (LPCSTR)inFace;
#endif

  data.faceDesc = This;
  data.master = inMaster;

  /* Enumerates all fonts in all character sets */
  lfont.lfCharSet = DEFAULT_CHARSET;
  lfont.lfFaceName[0] = '\0';

  if (EnumFontFamiliesEx(dc, &lfont, (FONTENUMPROC)__glcEnumFaceDescCreate,
                         (LPARAM)&data, 0)) {
